...
COPY public.products (id, user_id, name, title, description, iva, category_id, status, created_at) FROM stdin;
\.
...
COPY public.refresh_tokens (token_id, user_id, device_name, ip_address, user_agent, created_at, edited_at, revoked) FROM stdin;
\.
...
COPY public.users (id, user_id, first_name, last_name, username, password, email, telephone, status, profile, created_at, updated_at, role) FROM stdin;
\.
...
COPY public.variant_products (id, created_at, product_id, type_sale, price, price_iva, stock, sort, attributes, status, iva) FROM stdin;
\.
...
SELECT pg_catalog.setval('public.direcction_id_seq', 1, false);
...
SELECT pg_catalog.setval('public.seq_retailer_id', 1, false);
...
SELECT pg_catalog.setval('public.seq_support_id', 1, false);
