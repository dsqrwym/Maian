...
import { HashWorkerData } from './hash-worker-pool.provider';
...
// 比较函数
async function compareWithBcrypt(
  string: string,
  hash: string,
): Promise<boolean> {
...
function hashWithCrypto(input: string | Buffer): string {
...
export default async function (
  ...
  if (data.type === 'hash') {
    ...
    return data.algorithm === 'bcrypt'
      ? await compareWithBcrypt(data.string, data.hashedString!)
      : compareWithCrypto(data.string, data.hashedString!);
  } else {
