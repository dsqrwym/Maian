...
class AuthViewModel(
    ...
    var forgotPasswordUiState by mutableStateOf(UiState.Idle)
    ...
    fun forgotPasswordButtonClicked() {
        if (forgotStep < maxStep && forgotPasswordButtonEnabled.value) {
            // 设置加载状态
            forgotPasswordUiState = UiState.Loading
            viewModelScope.launch {
                delay(1000)
                forgotPasswordUiState = UiState.Error
                delay(1000)
                forgotPasswordUiState = UiState.Success
                delay(1000)
                forgotPasswordUiState = UiState.Idle
                forgotStep++
            }
        }
    }
